...
async def orders_admin_patch_multiple(request: Request):
    ...
    try:
        ...
        result = await orders_service.bulk_update_locations(me["id"], orders_and_locations)
...
async def orders_admin_get(request: Request):
    ...
    filters = OrderFilter(
        ...
        filter_limit=int(request.query_params.get("filter_limit", 100)),
    )
    ...
    for order in orders:
        ...
        order["mag_location"] = order["magasin"]
